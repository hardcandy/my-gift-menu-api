...
「我的学习」是家庭圈专用的儿童学习记录模块，按圈子隔离数据。家庭圈家长组可新增、编辑、删除学习项和学习记录，并可将未订正记录一键标记为已订正；孩子组和亲友组默认仅查看。

新增表：

- `t_gift_study_item`：学习项配置，保存名称、适用学科、适用年级、字段配置和是否启用订正标记。
- `t_gift_study_record`：学习记录，保存孩子、年级、学科、学习项、成绩/结果、★/△ 标记、附件和备注。
